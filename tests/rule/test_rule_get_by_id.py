...
def test_id_found(mocker):
    ...
    repo.get_by_id_and_policy.return_value = Rule(
        id=0,
        policy_id=0,
        name="test_rule",
        order=0,
    ...
    rule = use_case.execute(0, 0, 0)

    assert rule
    assert rule.id == 0
    assert rule.policy_id == 0

    repo.get_by_id_and_policy.assert_called_once()


def test_id_not_found(mocker):
    """Testing a get by id where none is found"""
    repo = mocker.Mock(spec=RuleRepository)
    repo.get_by_id_and_policy.return_value = None

    policy_repo = mocker.Mock(spec=PolicyRepository)

    use_case = GetRuleByIdUC(repo, policy_repo)
    rule = use_case.execute(0, 0, 1)
